        ...
        <div class="show-comments">
            @if (!empty($comments))
                @foreach ($comments as $comment)
                    @php
                        $title = DB::table('contents')
                            ->where('id', $comment->content_id)
                            ->get();
                        $playlist = DB::table('playlists')
                            ->where('id', $title['0']->playlist_id)
                            ->get();
                    @endphp
                    <div class="box" style="order:-1">
                        <div class="content"><span>{{ date('Y-m-d', strtotime($comment->created_at)) }}</span>
                            <p> - {{ $title['0']->title }} - </p><a
                                href="{{ route('view-video-admin', [$playlist['0']->slug, $title['0']->slug]) }}">view content</a>
                        </div>
